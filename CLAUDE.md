...
**Every originally-scoped Phase 1 feature is built and verified on device.** In place: the scaffold (#3 — Expo SDK 54 managed + TypeScript + expo-router, rooted in `src/`, theme/token layer, two-tab shell), the domain models + seed JSON schema (#4), seed content for the two MVP books (#20), the repository + local-persistence layer (#22), the core screens — Library + Book detail (#7), Mention detail + notes (#8), the log-a-mention sheet + My stuff screen (#9) — plus add-a-book (#12), the `place` kind (#13), library search (#14), book removal (#34), logging a mention from My stuff (#27/#39), and book-level notes (#15).

**Phase 1 gained new scope in August 2026** and is not closed: onboarding/first-run, which the design handoff never covered (#44 design → #45 build), and distinguishing the curated seed books from the reader's own library (#46). **GitHub issues are the live tracker** — check open issues before starting, and keep this paragraph roughly current as they close.
